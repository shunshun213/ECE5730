...
// 生成swoop和chirp的声音
#define SWOOP_F_CENTER   1740.0f  // 起/止频率
#define SWOOP_F_DEV       260.0f  // 偏移幅度（中点峰值=中心+偏移=2000Hz）
#define CHIRP_F0   2000.0f   // 起始频率 (Hz)
#define CHIRP_F1   7000.0f   // 结束频率 (Hz)
typedef enum { MODE_BEEP = 0, MODE_SWOOP = 1, MODE_CHIRP = 2 } sound_mode_t;
volatile sound_mode_t current_mode = MODE_BEEP;  // 默认 beep
...
volatile unsigned int phase_incr_main_0 = (400.0*two32)/Fs ;//只是建立一个变量，并且给他一个初始值，ISR内赋值，是不断更新这个变量，让频率随时间变化。
...
//#define ATTACK_TIME  800   // 或 1000 包络时间长，声音软
//#define DECAY_TIME   800   // 或 1000
#define SUSTAIN_TIME            10000
#define BEEP_DURATION           10500
#define BEEP_REPEAT_INTERVAL    50000  // ≈ 210 ms @ 50 kHz
// #define BEEP_DURATION 6500   // ≈ 130 ms @ 50 kHz
...
static void alarm_irq(void) {
    ...
    if (STATE_0 == 0) {
        ...
        float freq;
        // 归一化进度 0..1（供 swoop/chirp 使用）
        float u = (float)count_0 / (float)BEEP_DURATION;

        switch (current_mode) {
            case MODE_BEEP:
                // 固定频率（举例 400 Hz；也可用你原先初始化的相位步长）
                freq = 400.0f;
                break;

            case MODE_SWOOP:
                // 半周期正弦的“先升后降” swoop：1740 → 2000 → 1740
                // 可调：中心 1740、偏移 260
                freq = 1740.0f - 260.0f * sinf((float)M_PI * u);
                break;

            case MODE_CHIRP:
                // 二次上扫 chirp：2000 → 7000（前慢后快）
                // 线性上扫可用：freq = 2000.0f + (7000.0f-2000.0f) * u;
                freq = 2000.0f + (7000.0f - 2000.0f) * (u * u);
                break;
        }

        // 频率 → DDS 相位步长（two32=4294967296.0；Fs=50000）
        phase_incr_main_0 = (unsigned int)((freq * two32) / Fs);


        //更新phase_incr_main_0 (swoop) 
        //float t = (float)count_0 / (float)BEEP_DURATION;   // 0..1
        //float freq = 1740.0f - 260.0f * sinf(M_PI * t);
        //phase_incr_main_0 = (unsigned int)((freq * two32) / Fs);

        //更新phase_incr_main_0 (swoop)的另一种方法--线性上扫再线性下扫
        //float freq;
        //int half = (BEEP_DURATION >> 1); // 一半长度

        //if (count_0 <= half) {
        //    // 0 → half：1740 线性升到 2000
        //    float u = (float)count_0 / (float)half;                // 0..1
        //    freq = 1740.0f + (2000.0f - 1740.0f) * u;
        //} else {
        //    // half → 终点：2000 线性降回 1740
        //    float u = (float)(count_0 - half) / (float)half;       // 0..1
        //    freq = 2000.0f - (2000.0f - 1740.0f) * u;
        //}
        //phase_incr_main_0 = (unsigned int)((freq * two32) / Fs);

        //更新phase_incr_main_0 (chirp)
        //float u = (float)count_0 / (float)BEEP_DURATION;  // 0..1
        //float freq = CHIRP_F0 + (CHIRP_F1 - CHIRP_F0) * (u * u); // 二次上升
        //phase_incr_main_0 = (unsigned int)((freq * two32) / Fs);
        ...
        switch (key_status) {
            ...
            case Maybe_Pressed:
                ...
                else 
                {
                    ...
                    if (i == 1) {                 // “1” 键
                        current_mode = MODE_SWOOP;
                    } else if (i == 2) {          // “2” 键
                        current_mode = MODE_CHIRP;
                    } else {                      // 其它键
                        current_mode = MODE_BEEP;
                    }
